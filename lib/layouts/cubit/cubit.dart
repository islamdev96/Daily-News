// ignore_for_file: non_constant_identifier_names, invalid_return_type_for_catch_error, avoid_print
...
class AppCubit extends Cubit<AppStates> {
  ...
  void getNews({required String country, required String category}) {
    ...
    DioHelper.getDataFromAPi(methodUrl: 'v2/top-headlines', query: {
      'country': country,
      'category': category,
  ...
  void getSearchData({required String query}) {
    search = [];
    emit(LoadingDuringGettingDataFromApi());
    DioHelper.getDataFromAPi(methodUrl: 'v2/everything', query: {
      'q': query,
      'apiKey': '8f4c8e5d17f541f99033ee5eb5290ed1',
    }).then((value) {
  ...
  void createDatabase() async {
    db = await openDatabase('news.db', version: 1,
      ...
      database
          .execute(
              "CREATE TABLE news (id INTEGER PRIMARY KEY, title TEXT, urlImage TEXT, publishedAt TEXT)")
          .then((value) {
  ...
  void InsertTODatabase({
    ...
    db?.transaction((txn) async {
      txn.rawInsert(
          'INSERT INTO news(title,urlImage,publishedAt) VALUES ("$title","$urlToImage","$publishedAt")');
    }).then((val) {
  ...
  void DeleteDatebase({required int id}) {
    db?.rawUpdate('DELETE FROM news WHERE id = ?', [id]).then((value) {
